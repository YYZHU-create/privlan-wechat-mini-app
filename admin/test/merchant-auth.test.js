...
test("registers a complete merchant workspace and enforces credentials and session revocation", async () => {
  ...
  try {
    const created = await service.register({ login: "merchant@example.com", password: "correct-password", storeName: "Example Atelier", template: "blank" });
    ...
    const loggedIn = await service.login({ login: "merchant@example.com", password: "correct-password" });
    ...
    assert.doesNotMatch(stored, /correct-password/);
