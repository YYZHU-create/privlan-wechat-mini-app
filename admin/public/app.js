...
createApp({
  setup() {
    ...
    const mvpFeatureFlags = Object.freeze({ orders: false, marketing: true, analytics: false, workflow: true });
    const mvpViews = new Set(["overview", "editor", "products", "categories", "media", "customers", "membership", "appointments", "marketing", "workflow", "ai-service", "ai-template", "channels", "account", "settings", "theme"]);
    ...
    const workflow = reactive({ loading: false, error: "", definitions: [], runs: [], capabilities: [], dialog: false, form: { workflowKey: "", name: "", tasksText: "" }, saving: false });
    ...
    const navGroups = computed(() => [
      ...
      { id: "operations", label: "经营中心", items: [
        ...
        { id: "marketing", label: "营销中心", icon: "ph:megaphone" },
        { id: "workflow", label: "Workflow", icon: "ph:git-branch" },
    ...
    const viewTitle = computed(() => {
      ...
      return ({
        ...
        "ai-service": "智能客服", "ai-template": "AI 模板工作室", workflow: "Workflow 自动化", marketing: "营销中心", channels: "小程序预览", account: "账户与订阅", settings: "全局设置"
    ...
    async function loadWorkflow() {
      workflow.loading = true; workflow.error = "";
      try {
        const [definitions, runs, capabilities] = await Promise.all([apiJson("/v1/workflow-definitions"), apiJson("/v1/workflow-runs"), apiJson("/v1/workflow-capabilities")]);
        workflow.definitions = definitions || []; workflow.runs = runs || []; workflow.capabilities = capabilities || [];
      } catch (error) { workflow.error = error.message || "Workflow 数据读取失败"; }
      finally { workflow.loading = false; }
    }
    async function createWorkflow() {
      const key = String(workflow.form.workflowKey || "").trim(); const name = String(workflow.form.name || "").trim();
      const tasks = String(workflow.form.tasksText || "").split(",").map((title, index) => ({ key: `task-${index + 1}`, type: "manual", title: title.trim() })).filter(item => item.title);
      if (!/^[A-Za-z0-9][A-Za-z0-9_.-]{0,79}$/.test(key) || !name || !tasks.length) { workflow.error = "请填写有效的标识、名称和至少一个任务"; return; }
      workflow.saving = true; workflow.error = "";
      try { await apiJson("/v1/workflow-definitions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowKey: key, name, definition: { tasks } }) }); workflow.form = { workflowKey: "", name: "", tasksText: "" }; workflow.dialog = false; await loadWorkflow(); toast("Workflow 已创建", name); }
      catch (error) { workflow.error = error.message || "Workflow 创建失败"; } finally { workflow.saving = false; }
    }
    async function setWorkflowStatus(item) { const status = item.status === "active" ? "archived" : "active"; try { await apiJson(`/v1/workflow-definitions/${encodeURIComponent(item.workflowKey)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) }); await loadWorkflow(); } catch (error) { workflow.error = error.message || "Workflow 状态更新失败"; } }
    ...
    async function checkMerchantSession(loadWorkspace = true) {
      ...
      try {
        ...
        if (loadWorkspace) { await Promise.all([loadConfig(), loadPlatform(), loadSubscription(), loadProfile(), loadBusinessTemplates()]); if(currentView.value==="overview"||currentView.value==="customers"||currentView.value==="membership"||currentView.value==="appointments")await loadAppointmentWorkspace(); else if(currentView.value==="marketing")await loadMarketing(); else if(currentView.value==="workflow")await loadWorkflow(); else if(currentView.value==="ai-template")await loadAiTemplateStudio(); else if(currentView.value==="editor")await loadAppointmentServices(); loadCart(); }
    ...
    <div v-else class="app-shell" :class="{'editor-mode': currentView === 'editor', 'preview-command-shell': currentView === 'channels', 'has-subscription-banner': account.subscription?.status==='expired' || account.subscription?.status==='inactive' || (account.subscription?.remainingDays!==null && account.subscription?.remainingDays<=3)}">
      ...
      <div class="workspace" :class="{'sidebar-collapsed':sidebarCollapsed,'sidebar-mobile-open':mobileSidebarOpen}">
        ...
        <main id="main-content" class="workspace-main" tabindex="-1">
          ...
          <section v-else-if="currentView === 'workflow'" class="management">
            <div class="management-header"><div><span class="eyebrow">WORKFLOW AUTOMATION</span><h1>Workflow 自动化</h1><p>配置触发、任务顺序与执行结果。</p></div><button class="btn primary" @click="workflow.dialog=true">新建 Workflow</button></div>
            <div v-if="workflow.loading" class="appointment-loading">正在读取 Workflow…</div><div v-else-if="workflow.error" class="form-error" role="alert">{{ workflow.error }}</div>
            <div v-else class="marketing-lifecycle-grid"><section class="data-card"><div class="atelier-panel-head"><div><h2>定义</h2><p>{{ workflow.definitions.length }} 个 Workflow</p></div></div><div v-if="!workflow.definitions.length" class="empty-compact">暂无 Workflow 定义</div><article v-for="item in workflow.definitions" :key="item.id" class="marketing-row"><span><strong>{{ item.name }}</strong><small>{{ item.workflowKey }} · {{ item.latestVersionStatus || 'draft' }} · {{ item.taskCount }} 个任务</small></span><button class="btn small" @click="setWorkflowStatus(item)">{{ item.status === 'active' ? '归档' : '启用' }}</button></article></section><section class="data-card"><div class="atelier-panel-head"><div><h2>执行记录</h2><p>{{ workflow.runs.length }} 条</p></div></div><div v-if="!workflow.runs.length" class="empty-compact">暂无执行记录</div><article v-for="run in workflow.runs" :key="run.id" class="marketing-row"><span><strong>{{ run.workflowKey }}</strong><small>{{ run.status }} · {{ run.failedTasks || 0 }} 个失败任务</small></span></article></section></div>
            <div v-if="workflow.dialog" class="drawer-backdrop" @click.self="workflow.dialog=false"><aside class="appointment-detail-drawer" role="dialog" aria-modal="true"><header><div><span class="eyebrow">WORKFLOW</span><h2>新建 Workflow</h2></div><button class="icon-btn" aria-label="关闭" @click="workflow.dialog=false">×</button></header><form class="booking-rules" @submit.prevent="createWorkflow"><label>标识<input v-model.trim="workflow.form.workflowKey" required placeholder="customer-onboarding"></label><label>名称<input v-model.trim="workflow.form.name" required></label><label>任务（逗号分隔）<input v-model.trim="workflow.form.tasksText" required placeholder="收集信息,人工审核"></label><div class="management-actions"><button type="button" class="btn" @click="workflow.dialog=false">取消</button><button type="submit" class="btn primary" :disabled="workflow.saving">{{ workflow.saving ? '创建中…' : '创建' }}</button></div></form></aside></div>
          </section>
