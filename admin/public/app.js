...
createApp({
  setup() {
    ...
    const mvpViews = new Set(["overview", "editor", "products", "categories", "media", "customers", "appointments", "ai-service", "channels", "account", "settings", "theme"]);
    ...
    const appointmentWorkspace = reactive({ loading: false, error: "", tab: "appointments", customerSection: "all", stats: { today: 0, week: 0, pending: 0, customers: 0 }, customerStats: { total: 0, customers: 0, members: 0, new30Days: 0 }, appointments: [], customers: [], customerPage: 1, customerPageSize: 25, customerTotal: 0, services: [], advisors: [], filters: { q: "", status: "", serviceId: "", advisorId: "", dateFrom: "", dateTo: "", identity: "", source: "", sort: "activity" } });
    const appointmentDrawer = reactive({ open: false, loading: false, item: null, error: "" });
    ...
    const bookingSettings = reactive({ open: false, loading: false, saving: false, section: "rules", error: "", rules: { timezone: "Asia/Shanghai", slotIntervalMinutes: 30, defaultBufferMinutes: 1, maxAdvanceDays: 30, bookingEnabled: true }, hours: [], services: [], advisors: [] });
    let historyTimer;
    ...
    const navItems = computed(() => [
      ...
      ...(cfg.value?.features?.appointments === false ? [] : [{ id: "appointments", label: "预约", icon: "ph:calendar-check" }]),
    ...
    async function loadAppointmentWorkspace() {
      ...
      try {
        ...
        Object.assign(appointmentWorkspace.stats, stats || {}); Object.assign(appointmentWorkspace.customerStats, customerStats || {}); appointmentWorkspace.appointments = appointments || []; appointmentWorkspace.customers = customerResult?.items || []; appointmentWorkspace.customerTotal = Number(customerResult?.total || 0); appointmentWorkspace.services = services || []; appointmentWorkspace.advisors = advisors || []; customerAdmin.tags=tags||[]; customerAdmin.program=program||{enabled:false,points_enabled:false}; customerAdmin.levels=levels||[];
      } catch (error) { appointmentWorkspace.error = error.message; }
      finally { appointmentWorkspace.loading = false; }
    }

    async function openAppointment(item) { appointmentDrawer.open = true; appointmentDrawer.loading = true; appointmentDrawer.error = ""; appointmentDrawer.item = null; try { appointmentDrawer.item = await apiJson(`/v1/appointments/${encodeURIComponent(item.id)}`); } catch(error){ appointmentDrawer.error=error.message; } finally { appointmentDrawer.loading=false; } }
    ...
    async function openCustomer(item) { customerDrawer.open=true;customerDrawer.loading=true;customerDrawer.error="";customerDrawer.tab="overview";customerDrawer.item=null;try{customerDrawer.item=await apiJson(`/v1/customers/${encodeURIComponent(item.id)}`);}catch(error){customerDrawer.error=error.message;}finally{customerDrawer.loading=false;} }
    ...
    async function openBookingSettings() { bookingSettings.open=true;bookingSettings.loading=true;bookingSettings.error="";try{const [rules,hours,services,advisors]=await Promise.all([apiJson("/v1/appointment-settings"),apiJson("/v1/appointment-business-hours"),apiJson("/v1/appointment-services"),apiJson("/v1/appointment-advisors")]);Object.assign(bookingSettings.rules,rules);bookingSettings.hours=hours;bookingSettings.services=(services||[]).map(item=>({...item,bufferMode:item.bufferMinutesOverride===null?"inherit":"custom"}));bookingSettings.advisors=advisors;}catch(error){bookingSettings.error=error.message;}finally{bookingSettings.loading=false;} }
    ...
    async function submitAuth() {
      ...
      try {
        ...
        await Promise.all([loadConfig(), loadPlatform(), loadSubscription(), loadProfile(), loadBusinessTemplates()]); loadCart();
    ...
    async function checkMerchantSession(loadWorkspace = true) {
      ...
      try {
        ...
        if (loadWorkspace) { await Promise.all([loadConfig(), loadPlatform(), loadSubscription(), loadProfile(), loadBusinessTemplates()]); if(currentView.value==="customers"||currentView.value==="appointments")await loadAppointmentWorkspace(); loadCart(); }
    ...
    function switchView(id) {
      ...
      if (id === "customers" || id === "appointments") { appointmentWorkspace.tab = id === "appointments" ? "appointments" : "customers"; loadAppointmentWorkspace(); }
    ...
    return {
      ...
      loadAppointmentWorkspace, openAppointment, updateAppointmentStatus, openCustomer, addCustomerNote, adjustCustomerPoints, createCustomerTag, saveMembershipProgram, saveMembershipLevel, openBookingSettings, saveBookingRules, addBusinessWindow, removeBusinessWindow, saveBusinessHours, addBookingService, saveBookingService, addBookingAdvisor, saveBookingAdvisor, formatAppointmentTime, sourceLabel
    };
  },
    ...
    <div v-else class="app-shell" :class="{'editor-mode': currentView === 'editor', 'preview-command-shell': currentView === 'channels', 'has-subscription-banner': account.subscription?.status==='expired' || account.subscription?.status==='inactive' || (account.subscription?.remainingDays!==null && account.subscription?.remainingDays<=3)}">
      ...
      <div class="workspace" :class="{'sidebar-collapsed':sidebarCollapsed,'sidebar-mobile-open':mobileSidebarOpen}">
        ...
        <main id="main-content" class="workspace-main" tabindex="-1">
          ...
          <section v-else-if="currentView === 'settings'" class="management">
            ...
            <section class="theme-card tabbar-settings-card"><div class="settings-card-header"><div><h2>底部导航</h2><p>设置 2–5 个可见导航项，可调整顺序、页面目标与可选突出项。</p></div><button type="button" class="btn small" :disabled="cfg.tabBar.items.length>=5" @click="addTabItem"><iconify-icon class="icon" icon="ph:plus"></iconify-icon>添加导航</button></div>
              <div class="tabbar-settings-list">
                <article v-for="(item,index) in cfg.tabBar.items" :key="item.id" class="tabbar-setting-row" :class="{center:cfg.tabBar.featuredItemId===item.id,disabled:item.visible===false}">
                  <div class="tabbar-setting-title"><span class="tabbar-setting-index">{{ index + 1 }}</span><div><strong>{{ item.text }}</strong><small>{{ item.visible===false ? '已隐藏' : cfg.tabBar.featuredItemId===item.id ? '突出导航项' : '普通导航项' }}</small></div><div class="row-actions"><button type="button" title="上移" :disabled="index===0" @click="moveTabItem(index,-1)"><iconify-icon class="icon" icon="ph:arrow-up"></iconify-icon></button><button type="button" title="下移" :disabled="index===cfg.tabBar.items.length-1" @click="moveTabItem(index,1)"><iconify-icon class="icon" icon="ph:arrow-down"></iconify-icon></button><button type="button" title="显示或隐藏" @click="toggleTabVisibility(item)"><iconify-icon class="icon" :icon="item.visible===false?'ph:eye-slash':'ph:eye'"></iconify-icon></button><button type="button" title="设为突出项" @click="toggleFeaturedTab(item)"><iconify-icon class="icon" icon="ph:star"></iconify-icon></button><button type="button" title="删除" class="danger" @click="removeTabItem(index)"><iconify-icon class="icon" icon="ph:trash"></iconify-icon></button></div></div>
                  <div class="tabbar-setting-fields">
                    ...
                    <div class="field"><label>跳转页面</label><select v-model="item.page"><option v-for="page in pageDefinitions.filter(page=>page.tab)" :key="page.id" :value="page.id">{{ page.name }}</option></select></div>
                    <template v-if="cfg.tabBar.featuredItemId!==item.id">
              ...
              <p class="tabbar-settings-note"><iconify-icon class="icon" icon="ph:info"></iconify-icon>静态图片支持拖动取景并生成 512 × 512 WebP；中间主按钮也可使用小于 1.2 MB 的 GIF，动画会完整保留且不能裁切。保存后点击“生成预览”才会写入小程序开发文件。</p>
            </section>
          </section>
      ...
      <template v-if="homeNavOpen">
        ...
        <aside class="drawer home-nav-drawer">
          <div class="drawer-header"><div><h2>首页导航</h2><p>管理首页顶部的频道名称和顺序。</p></div><button type="button" class="icon-btn" aria-label="关闭首页导航设置" @click="finishHomeNavigation"><iconify-icon class="icon" icon="ph:x"></iconify-icon></button></div>
