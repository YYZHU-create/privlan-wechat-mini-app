                  ...
                  <div class="mobile-page" :style="{'--page-bg': cfg.theme.colors.bgPrimary, '--page-secondary': cfg.theme.colors.bgSecondary, '--page-text': cfg.theme.colors.textPrimary, '--page-muted': cfg.theme.colors.textSecondary, '--page-accent': cfg.theme.colors.accent, '--page-border': cfg.theme.colors.border}">
                    ...
                    <template v-for="(section, index) in sections" :key="section.id">
                      ...
                      <section class="page-section" :class="{'is-selected': selectedId === section.id, 'is-hidden': !section.enabled}" :data-label="blockLabel(section)" :style="sectionStyle(section)" @click.stop="selectedId = section.id">
                        ...
                        <div v-else-if="section.type === 'member-banner'" class="member-section"><img v-if="section.props.useBrandLogo !== false" class="member-brand-logo" src="/mp-images/privlan-ai-logo-white.png" alt="PRIVLAN"><h3 v-else>{{ section.props.title || cfg.brand.name }}</h3><p>{{ section.props.subtitle || cfg.brand.slogan }}</p><div class="benefits"><div v-for="benefit in cfg.memberBenefits.slice(0,4)" :key="benefit.text" class="benefit"><img :src="mpUrl(benefit.icon)" :alt="benefit.text"><span>{{ benefit.text }}</span></div></div></div>
            ...
            <aside class="side-panel right">
              ...
              <div v-else class="inspector-content">
                <template v-if="inspectorTab === 'content'">
                  ...
                  <div v-else-if="selectedSection.type === 'appointment-form'" class="field-group"><div class="field-title">预约字段</div><p class="field-help">字段顺序按预约流程固定，门店、顾问和时段选项由飞书实时提供。</p><div v-for="field in [{key:'showName',label:'姓名'},{key:'showPhone',label:'联系电话'},{key:'showService',label:'预约服务'},{key:'showStore',label:'到店门店'},{key:'showDate',label:'预约日期'},{key:'showTime',label:'预约时间'},{key:'showAdvisor',label:'专属顾问'}]" :key="field.key" class="toggle-row"><span>{{ field.label }}</span><button class="switch" :class="{on:selectedSection.props[field.key] !== false}" @click="selectedSection.props[field.key] = selectedSection.props[field.key] === false"></button></div></div>
