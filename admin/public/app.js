...
createApp({
  setup() {
    ...
    const navItems = [
      ...
      { id: "appointments", label: "预约", icon: "ph:calendar-check" },
    ...
    async function checkMerchantSession(loadWorkspace = true) {
      ...
      try {
        ...
        if (loadWorkspace) { await Promise.all([loadConfig(), loadPlatform(), loadSubscription()]); if(currentView.value==="customers"||currentView.value==="appointments")await loadAppointmentWorkspace(); loadCart(); }
