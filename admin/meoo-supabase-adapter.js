...
function createSupabaseAdapter({ url = process.env.SUPABASE_URL, serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY, fetchImpl = globalThis.fetch, table = "b1_customer_tags" } = {}) {
  if (!url || !serviceRoleKey || typeof fetchImpl !== "function") throw new Error("Supabase server configuration is required");
