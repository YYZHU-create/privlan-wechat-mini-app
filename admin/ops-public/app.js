...
createApp({
  setup() {
    ...
    const view = ref(new URLSearchParams(location.search).get("view") || "overview");
    const data = reactive({ metrics: {}, tenants: [], plans: [], subscriptions: [], providerCatalog: [], platformConnections: [], tenantConnections: [], aiPolicies: [], aiUsage: [], publishJobs: [], featureFlags: [], supportTickets: [], incidents: [], impersonationSessions: [], auditEvents: [], workspace: null });
    const error = ref("");
    ...
    const nav = [
      ...
      { id: "ai", label: "AI 服务", icon: "ph:circuitry" },
      { id: "publishing", label: "发布", icon: "ph:rocket-launch" },
      { id: "flags", label: "功能开关", icon: "ph:toggles" },
      { id: "support", label: "工单与事件", icon: "ph:lifebuoy" },
    ...
    function handleKeydown(event) {
      ...
      connectionForm.open = false; ticketForm.open = false; incidentForm.open = false; impersonationForm.open = false;
    }

    onMounted(() => { checkSession(); window.addEventListener("keydown", handleKeydown); });
    onUnmounted(() => window.removeEventListener("keydown", handleKeydown));
    return { session, loading, login, view, data, error, notices, nav, title, connectionForm, ticketForm, incidentForm, impersonationForm, busy, activeImpersonation, signIn, signOut, loadData, switchView, formatNumber, formatMoney, formatDate, statusLabel, applyProviderPreset, createPlatformConnection, testPlatformConnection, rotatePlatformSecret, togglePlatformConnection, deletePlatformConnection, retryPublish, rollbackPublish, updateTenant, updatePlan, toggleFlag, createTicket, updateTicket, createIncident, openImpersonation, startImpersonation, endImpersonation };
  },
  template: `
    ...
    <div v-else class="ops-shell">
      ...
      <div class="ops-body"><aside class="ops-sidebar"><nav aria-label="运营导航"><button v-for="item in nav" :key="item.id" type="button" :class="{active:view===item.id}" :aria-current="view===item.id?'page':null" @click="switchView(item.id)"><iconify-icon :icon="item.icon" aria-hidden="true"></iconify-icon><span>{{ item.label }}</span></button></nav><div class="ops-sidebar-foot"><span class="health-dot"></span><span><strong>本地控制面</strong><small>生产环境需接入 KMS / PostgreSQL</small></span></div></aside>
        <main id="ops-main" class="ops-main" tabindex="-1"><div v-if="error" class="page-error" role="alert"><span>{{ error }}</span><button type="button" @click="loadData">重试</button></div>
          <section v-if="view==='overview'" class="ops-page"><header class="page-head"><div><span class="eyebrow">SYSTEM OVERVIEW</span><h1>平台运行概览</h1><p>只展示运营和服务健康数据。敏感客户资料与完整聊天正文默认不可见。</p></div><span class="state-chip success"><i></i>核心服务正常</span></header><div class="metric-grid"><article><span>租户</span><strong>{{ formatNumber(data.metrics.tenants) }}</strong><small>{{ data.metrics.activeTenants }} 个正常 · {{ data.metrics.trials }} 个试用</small></article><article><span>发布成功率</span><strong>{{ data.metrics.publishSuccessRate }}%</strong><small>{{ data.publishJobs.length }} 个发布任务</small></article><article><span>平台 AI 用量</span><strong>{{ formatNumber(data.metrics.aiPoints) }}</strong><small>{{ data.metrics.aiErrors }} 个失败请求</small></article><article><span>待处理</span><strong>{{ data.metrics.openTickets + data.metrics.activeIncidents }}</strong><small>{{ data.metrics.openTickets }} 工单 · {{ data.metrics.activeIncidents }} 事件</small></article></div><div class="overview-layout"><section class="panel"><header><div><span class="eyebrow">TENANT PULSE</span><h2>租户状态</h2></div><button type="button" class="text-btn" @click="switchView('tenants')">查看全部</button></header><div class="tenant-pulse"><article v-for="tenant in data.tenants" :key="tenant.id"><span class="tenant-monogram">{{ tenant.name.slice(0,1) }}</span><span><strong>{{ tenant.name }}</strong><small>{{ tenant.id }}</small></span><span class="state-chip" :class="tenant.status==='active'?'success':'warning'">{{ statusLabel(tenant.status) }}</span></article></div></section><section class="panel"><header><div><span class="eyebrow">OPERATIONS</span><h2>服务健康</h2></div></header><dl class="health-list"><div><dt>商户编辑器</dt><dd><i></i>正常</dd></div><div><dt>模型连接</dt><dd :class="{warning:!data.platformConnections.length}"><i></i>{{ data.platformConnections.length ? data.platformConnections.length + ' 个平台模型' : '未配置' }}</dd></div><div><dt>发布队列</dt><dd><i></i>{{ data.publishJobs.length ? '可追踪' : '等待任务' }}</dd></div><div><dt>活跃异常</dt><dd :class="{warning:data.metrics.activeIncidents}"><i></i>{{ data.metrics.activeIncidents || '无' }}</dd></div></dl></section></div></section>
          <section v-else-if="view==='tenants'" class="ops-page"><header class="page-head"><div><span class="eyebrow">TENANT CONTROL</span><h1>租户与工作区</h1><p>运营人员默认只查看状态；需要处理具体问题时，必须创建限时、可审计的代操作会话。</p></div></header><section class="panel table-panel"><table><thead><tr><th>租户</th><th>套餐</th><th>状态</th><th>渠道</th><th>操作</th></tr></thead><tbody><tr v-for="tenant in data.tenants" :key="tenant.id"><td><strong>{{ tenant.name }}</strong><small>{{ tenant.id }}</small></td><td><select :value="tenant.planId" aria-label="租户套餐" @change="updateTenant(tenant,{planId:$event.target.value})"><option v-for="plan in data.plans" :key="plan.id" :value="plan.id">{{ plan.name }}</option></select></td><td><select :value="tenant.status" aria-label="租户状态" @change="updateTenant(tenant,{status:$event.target.value})"><option value="trial">试用</option><option value="active">正常</option><option value="past_due">欠费</option><option value="suspended">暂停</option><option value="closed">关闭</option></select></td><td>{{ data.workspace?.channelMode==='shared'?'共享 AppID':'独立 AppID' }}</td><td><button type="button" class="btn small" @click="openImpersonation(tenant)"><iconify-icon icon="ph:eye"></iconify-icon>授权代操作</button></td></tr></tbody></table></section></section>
