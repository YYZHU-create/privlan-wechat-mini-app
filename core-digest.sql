select md5(
  ...
  coalesce((select string_agg(format('%s:%s',routine_name,external_language), '|' order by routine_name,external_language) from information_schema.routines where routine_schema='public' and routine_name <> 'atelier_create_appointment'),'')
) as core_schema_digest;
