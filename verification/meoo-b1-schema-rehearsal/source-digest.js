...
(async () => { const client = new Client({ connectionString: process.env.ATELIER_REAL_POSTGRES_URL }); await client.connect(); console.log((await client.query(sql)).rows[0].schema_digest); await client.end(); })().catch(error => { console.error(error.message); process.exit(1); });
