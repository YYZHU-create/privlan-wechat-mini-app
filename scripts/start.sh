...
if [ -z "${ATELIER_GIT_SHA:-}" ] && [ -f /code/.release-sha ]; then ATELIER_GIT_SHA="$(cat /code/.release-sha)"; fi
if [ -z "${ATELIER_GIT_SHA:-}" ] && [ -f /code/release-sha ]; then ATELIER_GIT_SHA="$(cat /code/release-sha)"; fi
...
if [ -z "${ATELIER_GIT_BRANCH:-}" ] && [ -f /code/.release-branch ]; then ATELIER_GIT_BRANCH="$(cat /code/.release-branch)"; fi
if [ -z "${ATELIER_GIT_BRANCH:-}" ] && [ -f /code/release-branch ]; then ATELIER_GIT_BRANCH="$(cat /code/release-branch)"; fi
